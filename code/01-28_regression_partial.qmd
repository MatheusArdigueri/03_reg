...
```{r reg exp boxplot}
ggplot(reg_dfw, mapping = aes(x = sr_ksha, y = yield_mgha)) +
  theme_classic()+
  geom_boxplot()
...
mod1_int <- lmer(yield_mgha ~ 1 + (1 | rep ),
                 data = reg_dfw) #mixed effect models, allows us to incorporate random effects
...
if residuals are independently distributed around 0
```{r mod1 Standardized Residuals vs. Fitted}

ggplot(mod1_int_aug, mapping = aes(x =.fitted, y= .stdresid))+
  geom_point(size = 3,
             alpha = .7)+
  geom_hline(yintercept = 0, color = "red")+
  geom_smooth()

# The ideal would be residuals around 0, no clear pattern ... In this case they seems to be increasing.
```
...
# QQ plot helps us access if residuals are normally distributed. A normal distribution would follow the qq line. 
...
mod1_int_randeff <- ranef(mod1_int)[[1]] # Pull the first component of this list.
...
```{r nd}
nd <- data.frame(sr_ksha = seq(40,120,1)) # Min, Max, 1 by 1 
...
nd <- nd %>%
  mutate(mod1_yield_mgha = predict(mod1_int,
                                   nd,
                                   re.form = NA # Do not consider the random effect
                                   ))
...
ggplot(reg_dfw,
  ...
  geom_line(data = nd,
            aes(
                y= mod1_yield_mgha), color= "forestgreen") +
  geom_ribbon(data = nd,
...
mod3_quad <- lmer(yield_mgha ~ sr_ksha +
                    I(sr_ksha^2) + (1|rep),
                  data = reg_dfw) #~ means explained by. I() means that we want this paramer to be in power of two. 1|rep will see the random effect.
...
#Intercept, slope and breaking point. a,b,xs are not in df (how we want to call the int, slope and bp)
# Calculate the random effect taking a, b, and xs.
#We need to fix a startpoint for this model for each parameter that we are estimating.
...
ggplot(mod4_linp_randeff, 
  ...
  stat_qq()+
  stat_qq_line() +
  facet_wrap(~term) # If we need to take a look by term
...
IC_tab(mod1_int,
       ...
       mod4_linp)
```
Based on the above, model 4 (linear-plateau) had the lowest AIC and thus should be used to find the optimum level of seeding rate. For all of this parameters, the lowest values are the better. 
To be significant better, should be at least 2 points lower. And the simple, the best.

Delta AIC assigns the better model to 0, which turns easier to see that mod4 is the better one to find the optimum level of seeding rate.
...
ggplot(reg_dfw, aes(x = sr_ksha, y = yield_mgha))+
  ...
  geom_line(data = nd, aes(y = mod4_yield_mgha), 
            color = "orange",
            size = 1.5)  +
  geom_segment(x = 73.48,    # GeomSegment will indicate where is the maximum point.
               xend = 73.48,
               y = 0,
               yend = 13.45)+
  geom_segment(x = 0,
